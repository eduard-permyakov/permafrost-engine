...
def wrapper(ntabs):
    return textwrap.TextWrapper(width=80, 
        initial_indent=tab*ntabs,
        subsequent_indent=tab*ntabs)
...
for func in [getattr(pf, attr) for attr in dir(pf) if isinstance(getattr(pf, attr), types.BuiltinFunctionType)]:
    ...
    print "{0}".format(wrapper(1).fill(func.__doc__))
    print
...
for cls in [getattr(pf, attr) for attr in dir(pf) if isinstance(getattr(pf, attr), types.TypeType)]:
    ...
    print indented_border(1, char='-')
    print "{0}".format(wrapper(1).fill(cls.__doc__))
    print
    ...
    for field in fields:
        ...
        print "{0}".format(wrapper(2).fill(field.__doc__))
        print
