...
#define MEM_FILE_SUB MEM_SUB_GAME_TIMER_EVENTS
...
#undef PF_CALLOC
#undef PF_REALLOC
#define PF_MALLOC(_n)       PF_MALLOC_TAGGED((_n), MEM_SYS_GAME, MEM_SUB_GAME_TIMER_EVENTS)
#define PF_CALLOC(_c, _n)   PF_CALLOC_TAGGED((_c), (_n), MEM_SYS_GAME, MEM_SUB_GAME_TIMER_EVENTS)
#define PF_REALLOC(_p, _n)  PF_REALLOC_TAGGED((_p), (_n), MEM_SYS_GAME, MEM_SUB_GAME_TIMER_EVENTS)
...
static void timer_60hz_handler(void *unused1, void *unused2)
{
    ...
    if(s_num_60hz_ticks % 120 == 0)
        E_Global_Notify(EVENT_HALFHZ_TICK, NULL, ES_ENGINE);

    if(s_num_60hz_ticks % 60 == 0)
        E_Global_Notify(EVENT_1HZ_TICK, NULL, ES_ENGINE);

    if(s_num_60hz_ticks % 12 == 0)
        E_Global_Notify(EVENT_5HZ_TICK, NULL, ES_ENGINE);
