...
#define GPU_MEM_FILE_SYS GPU_MEM_SYS_GL_RENDER
#include "gl_mem.h"
#include "gl_perf.h"
#include "gl_state.h"
                        ...
                        mat4x4_t *model, const struct map *map)
{
    ...
    for(size_t i = 0, line_vbuff_idx = 0; i < *count; i++, line_vbuff_idx += 2) {
        ...
        line_vbuff[line_vbuff_idx] = (vec3_t){
            xz_positions[i].x,
            M_HeightAtPoint(map, 
                M_ClampedMapCoordinate(map, (vec2_t){base_ws_homo.x, base_ws_homo.z})) + 0.3, 
            xz_positions[i].z
        };
        line_vbuff[line_vbuff_idx + 1] = (vec3_t){
            tip.x,
            M_HeightAtPoint(map, 
                M_ClampedMapCoordinate(map, (vec2_t){tip_ws_homo.x, tip_ws_homo.z})) + 0.3, 
            tip.z
        };
